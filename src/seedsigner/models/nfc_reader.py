    ...
    def _decompress_seed_to_mnemonic(self, compressed_seed: bytes, seed_type: int, checksum: int) -> Optional[List[str]]:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Mnemonic generated by embit.bip39.mnemonic_from_bytes: {mnemonic}")
                logger.debug(f"Generated {len(mnemonic)} words, expected {num_words}")
                
                if len(mnemonic) != num_words:
                    logger.error(f"Generated mnemonic has {len(mnemonic)} words, expected {num_words}")
                    return None
                ...
                logger.debug(f"Generated checksum word: '{generated_checksum_word}' -> index {generated_checksum_index}")
                logger.debug(f"Stored checksum index: {checksum} -> word '{wordlist[checksum]}'")
                
                # チェックサム検証
                if generated_checksum_index == checksum:
                    logger.debug(f"✓ Checksum verification successful: {generated_checksum_index} == {checksum}")
                    return mnemonic
                else:
                    logger.error(f"✗ Checksum verification failed:")
                    logger.error(f"  Generated: {generated_checksum_index} -> '{generated_checksum_word}'")
                    logger.error(f"  Stored:    {checksum} -> '{wordlist[checksum]}'")
                    return None
